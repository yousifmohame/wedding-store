...
// Service categories based on your PDFs
...
function UploadServicePage() {
  ...
  useEffect(() => {
    if (categoryId) {
      ...
      // Initialize dynamic fields for the category
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Add validation for required dynamic fields based on currentCategory
      ...
      const providerData = {
        ...
        packages: [1, 2, 3, 4]
          .map(num => ({
            name: formData[`package${num}Name`]?.trim(),
            price: formData[`package${num}Price`] ? Number(formData[`package${num}Price`]) : null,
            description: formData[`package${num}Description`]?.trim(),
            // Add price per person for venues
            ...(currentCategory?.type === 'venue' && {
              pricePerPerson: formData[`package${num}PricePerPerson`] ? Number(formData[`package${num}PricePerPerson`]) : null
            })
          }))
          .filter(pkg => pkg.name && pkg.price !== null)
      };

      // Remove temporary package fields
      [1,2,3,4].forEach(num => {
        delete providerData[`package${num}Name`];
        delete providerData[`package${num}Price`];
        delete providerData[`package${num}Description`];
        delete providerData[`package${num}PricePerPerson`];
      });
                  ...
                  <div key={num} className="bg-gray-50 p-4 rounded-lg">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      <div>
                        ...
                        <input
                          ...
                          value={formData[`package${num}Name`] || ''}
                          onChange={handleInputChange}
                      ...
                      <div>
                        <label className="block text-gray-700 text-sm mb-1">الحد الأقصي لعدد الأشخاص</label>
                        <input
                          type="number"
                          name={`package${num}Price`}
                          value={formData[`package${num}Price`] || ''}
                          onChange={handleInputChange}
                          className="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-[#D4AF37] outline-none"
                          placeholder="الحد الأقصي"
                          min="0"
                        />
                      </div>
                      {/* Add price per person field for wedding venues */}
                      {currentCategory?.type === 'venue' && (
                        <div>
                          <label className="block text-gray-700 text-sm mb-1">سعر الفرد (ر.س)</label>
                          <input
                            type="number"
                            name={`package${num}PricePerPerson`}
                            value={formData[`package${num}PricePerPerson`] || ''}
                            onChange={handleInputChange}
                            className="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-[#D4AF37] outline-none"
                    ...
                    <div className="mt-2">
                      ...
                      <textarea
                        ...
                        value={formData[`package${num}Description`] || ''}
                        onChange={handleInputChange}
                        rows={2}
                        className="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-[#D4AF37] outline-none"
