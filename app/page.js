...
import { useAuth } from "../lib/auth"; // Updated auth hook

const EditSlideModal = dynamic(() => import("@/components/EditSlideModal"), {
  ssr: false,
});
    ...
    const fetchServices = async () => {
      ...
      try {
        ...
        const servicesData = querySnapshot.docs.map((doc) => ({
          id: doc.id, // This uses the Firestore document ID
  ...
  const handleSaveSlideChanges = async (slideData) => {
    try {
      if (isNewSlide) {
        ...
        const docRef = await addDoc(collection(db, "slides"), newSlideData);
        setSlides([...slides, { id: docRef.id, ...newSlideData }]);
              ...
              {filteredServices.length > 0 ? (
                filteredServices.map((service) => (
                  ...
                  >
                    ...
                    {isAdmin && (
                      <button
                        onClick={(e) => handleEditClick(service.id, e)}
